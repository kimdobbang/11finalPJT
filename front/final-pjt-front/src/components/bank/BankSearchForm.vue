<template>
  <div>
    ...
    <select v-model="selectedMetropolitan" @change="updateDistricts" id="metropolitan">
      ...
      <option v-for="(name, key) in administrativeDivisions" :key="key" :value="key">{{ name }}</option>
    </select>
    ...
    <label for="options">은 행</label>
      <select id="options" name="options">
          <option value="option1">국민</option>
          <option value="option2">신한</option>
          <option value="option3">우리</option>
          <option value="option4">하나</option>
      </select>
...
export default {
  ...
  methods: {
    updateDistricts() {
      this.districts = this.districtsData[this.selectedMetropolitan];
      this.selectedDistrict = ''; // Reset the district selection
    }
  }
};
</script>
